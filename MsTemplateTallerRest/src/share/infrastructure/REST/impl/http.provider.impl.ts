...
export class HttpUndiciProvider implements IHttpProvider {
     ...
     */
    async executeRest<R = any>(_requestConfig: IRequestConfigHttp, _task?: Etask | string): Promise<ApiResponseDto> {
        ...
        try {
            ...
            spanIn = this.apmService.startSpan(HttpUndiciProvider.name, 'Rest', 'executeRest', 'APM');
            ...
            result = new ApiResponseDto(httpResponse.statusCode, 'Response Rest', data, this.transactionId);
        } catch (error) {
            error['config'] = _requestConfig;
            result = new ApiResponseDto(error.statusCode, 'Error response consumo rest', error, this.transactionId);
        }
     ...
     * @returns 
     */
    private async buildAgent(_requestConfig: IRequestConfigHttp):Promise<object>{
